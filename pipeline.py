...
class FacePipeline:
    ...
    def verify(self, bgr_img, claimed_user_id: str):
        ...
        if emb is None:
            # notifica solo se vuoi
            if self.notifier:
                self.notifier.send(f"⚠️ Verifica fallita: nessun volto rilevato (claimed={claimed_user_id}).")
            return {"ok": False, **meta}

        ok, score = self.verifier.verify_claim(emb, claimed_user_id)

        # notifica solo in caso negativo
        if (not ok) and self.notifier:
            self.notifier.send(
                f"🚫 Accesso NEGATO. claimed={claimed_user_id} score={float(score):.3f} "
    ...
    def identify(self, bgr_img):
        ...
        if emb is None:
            if self.notifier:
                self.notifier.send("⚠️ Identificazione fallita: nessun volto rilevato.")
            return {"ok": False, "user": None, **meta}

        user, best, second_user, second_score = self.verifier.identify(emb)

        # notifica solo se NON riconosciuto
        if user is None and self.notifier:
            self.notifier.send(
                f"🚫 Sconosciuto/ambigua. best={float(best):.3f} "
